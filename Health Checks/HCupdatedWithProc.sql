IF OBJECT_ID('dbo.usp_BeginingHealthCheck') IS NULL
  EXEC ('CREATE PROCEDURE dbo.usp_BeginingHealthCheck AS RETURN 0;');
GO
ALTER PROCEDURE dbo.usp_BeginingHealthCheck
(@DB_NAME VARCHAR(100))
AS
...
DECLARE @AUTO_SHRINK varchar(200)

SET @AUTO_SHRINK = '12. Auto shrink databases -> '

DECLARE @MIN_MEM varchar(200)

SET @MIN_MEM = '13. Minimum size of server memory (MB) -> '

DECLARE @MAX_MEM varchar(200)

SET @MAX_MEM = '14. Maximum size of server memory (MB) -> '

DECLARE @PHY_MEM varchar(200)

SET @PHY_MEM = '15. Physical size of server memory (MB) -> '

DECLARE @COMPRESSION varchar(200)

SET @COMPRESSION = '16. Compression -> '

DECLARE @PROC varchar(200)

SET @PROC = '17. Processors -> '
...
--2. 
SELECT @WINDOWS_VERSION += SUBSTRING(@@VERSION,CHARINDEX(' on ',@@version,1) + 4,charindex('>',@@version,1)-CHARINDEX(' on ',@@version,1)-2)
--3.
...
ELSE
  select @RTO += concat((@rtotemp/60)+1,'h ', @rtotemp%60,'min (includes SQL Server installation and configuration, database restore and recovery)')
...
ORDER BY 1 DESC
SELECT @RPO += IIF(@RPOtemp IS NULL, 'NO BACKUPS FOUND', ('maximum data lost in case of server failure is ' + CAST(@RPOtemp/60 AS varchar(30)) + 'h ' + CAST(@RPOtemp%60 AS varchar(30))  + 'min'))
--5. --last check db 
...
--12 
DECLARE @AUTO_SHRINK_TEMP VARCHAR(MAX) = ''
SELECT @AUTO_SHRINK_TEMP = concat(name,' ') from sys.databases where name like '%xTrack%' and is_auto_shrink_on = 0 ; 
IF @AUTO_SHRINK_TEMP =''
	SELECT @AUTO_SHRINK += 'No databases'
ELSE
	SELECT @AUTO_SHRINK +=@AUTO_SHRINK_TEMP


--13
SELECT @MIN_MEM +=cast(value as varchar(100))
FROM [master]..[sysconfigures]
WHERE [comment] IN ('Minimum size of server memory (MB)')

--14
select @MAX_MEM += cast(value as varchar(100)) from master..sysconfigures
WHERE [comment] IN ( 'Maximum size of server memory (MB)')
...
SELECT @PHY_MEM += cast([Physical_Memory_In_MB] as varchar (1000))
...
select top 100 case
        ...
        end as Compression from msdb.dbo.backupset order by media_set_id desc
select @COMPRESSION += IIF((select sum(c) from #temp)=100,'Compression on (for the last 100 backups)','Compression off')

drop table #temp

--17
declare @CPUused int = (select count(*)/2 from sys.dm_os_schedulers where status like '%ONLINE%')
declare @CPUtotal int = (select count(*)/2 from sys.dm_os_schedulers)
select @PROC += CONCAT('There are ',cast(@cputotal as varchar (10)),' processors and ',cast(@CPUused as varchar(10)),' are ONLINE.')
...
--USAGE OF PROCEDURE
--exec sp_MSForEachDB @COMMAND1 = 'USE ? DECLARE @DB VARCHAR(100) = DB_NAME() EXEC [shop].[dbo].[usp_BeginingHealthCheck] @DB_NAME = @DB'


--ERASE YOUR TRACKS
--drop procedure [dbo].[usp_BeginingHealthCheck]