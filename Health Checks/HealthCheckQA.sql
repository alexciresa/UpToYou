...
SELECT
  ...
  + 4,
  CHARINDEX('(', @@VERSION, 1) + 1) --ALEX QA UPDATE: SELECT SUBSTRING(@@VERSION,CHARINDEX(' on ',@@version,1) + 4,charindex('>',@@version,1)-CHARINDEX(' on ',@@version,1)-2)
...
ELSE
  SELECT
    @RTO += ('1h ' + CAST(@rtoTemp AS varchar(30)) + 'min (includes SQL Server installation and configuration, database restore and recovery)')
	 --ALEX QA UPDATE: select @RTO += concat((@rtotemp/60)+1,'h ', @rtotemp%60,'min (includes SQL Server installation and configuration, database restore and recovery)')
...
IF SERVERPROPERTY('IsCluster') = 1 --ALEX QA UPDATE: IF SERVERPROPERTY('IsClustered') = 1
  SELECT
    @CLUSTER += 'True'
ELSE
  SELECT
