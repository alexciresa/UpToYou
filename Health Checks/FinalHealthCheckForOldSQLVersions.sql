--ATENTIE!!!! 2008 R2 SI 2008 R2 SP1 NU RECUNOASTE TABELELE PENTRU CALCULUI LAST UPDATE STATISTICS
--ATENTIE!!!! TREBUIE INLOCUITA BAZA DE DATE DIN "USE" DE LA PUNCTUL 7 IN FUNCTIE DE SITUATIE
...
SELECT TOP 1
  @RPOtemp =
  DATEDIFF(MINUTE,backup_finish_date,(
  SELECT  top 1 B1.BACKUP_FINISH_DATE 
  FROM MSDB..BACKUPSET B1
  WHERE B1.database_name = @db_name AND DATEDIFF(MONTH, B1.BACKUP_START_DATE, GETDATE()) <= 2
  AND B1.BACKUP_FINISH_DATE<B2.BACKUP_FINISH_DATE
  ORDER BY backup_finish_date DESC))  
--  (DATEDIFF(MINUTE, backup_finish_date, (LAG(backup_finish_date, 1, 0) OVER (
--  ORDER BY backup_finish_date DESC))))
FROM msdb..backupset B2
WHERE B2.database_name = @db_name AND DATEDIFF(MONTH, B2.BACKUP_START_DATE, GETDATE()) <= 2
ORDER BY 1 DESC
SELECT
  @RPO += ISNULL(('maximum data lost in case of server failure is '+CAST(@RPOtemp AS varchar(30)) + 'min' ) ,'NO BACKUPS FOUND')
--5. --last check db 
...
--  SELECT
    SELECT @AVAILABILITY_GROUPS += 'NONE FOUND'
--ELSE
--  SELECT
